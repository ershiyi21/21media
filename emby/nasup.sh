...
do  
    ...
    if [ 0 == $count ];then
       ...
       >> ${log_dir} 2>&1 &&
	     ...
	     do
	       echo "[$(date '+%Y-%m-%d %H:%M:%S')] 仍有文件存在，再次上传" >> ${log_dir}
	       /usr/bin/rclone move -v "$dir" "/" --config ${rclone_config_dir} \
         >> ${log_dir} 2>&1 &&
		     echo "[$(date '+%Y-%m-%d %H:%M:%S')] rclone再次上传完成" >> ${log_dir}   
	     done
	       echo "[$(date '+%Y-%m-%d %H:%M:%S')] 文件夹已无文件存在" >> ${log_dir}  
    else
	     echo "[$(date '+%Y-%m-%d %H:%M:%S')] rclone已在运行，避免死机，退出脚本" >> ${log_dir}
	     exit
    fi
